...
const EuroKidsNavbar = () => {
  ...
  const renderNavText = (name) => (
    <>
      <span className="block whitespace-nowrap  leading-tight">{name.split(' ')[0]}</span>
      ...
      {item.dropdown ? (
        <>
          <button
            ...
            className={` wh px-3 py-2 text-sm font-semibold rounded-md flex flex-col items-center 
              ${activeDropdown === index ? 'bg-[#E91E63] text-white' : 'text-gray-800 hover:bg-[#E91E63] hover:text-white'}`}
          >
            ...
            <div className="absolute left-0 mt-0 w-48 bg-white shadow-lg rounded-b-md z-20">
              {item.dropdown.map((subItem, subIndex) => (
                <NavLink
                  ...
                  to={subItem.path}
                  className={({ isActive }) =>
                    `block px-4 py-2 text-sm hover:bg-[#E91E63] hover:text-white ${isActive ? 'bg-red-100 text-[#E91E63]' : 'text-gray-800'}`
                  }
        ...
        <NavLink
          ...
          className={({ isActive }) =>
            ...
            ${isActive ? 'bg-[#E91E63] text-white' : 'text-gray-800 hover:bg-[#E91E63] hover:text-white'}`
          }
          onClick={handleLinkClick}
        >
          {renderNavText(item.name)}
    ...
    <div key={index}>
      {item.dropdown ? (
        <>
          <button
            ...
            className={`w-full px-4 py-3 text-left  flex justify-between items-center 
              ${activeDropdown === index ? 'bg-red-300 text-white' : 'text-gray-800 hover:bg-[#E91E63] hover:text-white'}`}
          >
            <div className="flex flex-col whitespace-nowrap text-[4px]  ">{renderNavText(item.name)}</div>
            <svg className={`h-4 w-4 transform ${activeDropdown === index ? 'rotate-180' : ''}`}>
            ...
            <div className="pl-6 bg-gray-50">
              {item.dropdown.map((subItem, subIndex) => (
                <NavLink
                  ...
                  to={subItem.path}
                  className={({ isActive }) =>
                    `block   py-3 text-sm ${isActive ? 'bg-red-100 text-[#E91E63]' : 'text-gray-800 hover:bg-[#E91E63] hover:text-white'}`
                  }
                  onClick={handleLinkClick}
                >
                  {subItem.name}
        ...
        <NavLink
          to={item.path}
          className={({ isActive }) =>
            `block px-4 py-3 ${isActive ? 'bg-[#E91E63] text-white' : 'text-gray-800 hover:bg-[#E91E63] hover:text-white'}`
          }
          onClick={handleLinkClick}
        >
          <div className="flex flex-col text-[12px]">{renderNavText(item.name)}</div>
        </NavLink>
      )}
    </div>
  );
    ...
    <nav className="bg-white shadow-md sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex justify-between items-center h-24">
          ...
          <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="lg:hidden p-2 text-gray">
...
>
  <div className="p-2 mb-2">
    ...
    <div className="  ">
      {navItems.map((item, index) => (
        <MobileNavItem key={index} item={item} index={index} className="whitespace-nowrap" />
      ))}
    </div>
